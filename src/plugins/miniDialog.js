...
;(function($,document,window){
    ...
    MiniDialog.prototype.setContent = function(msg){
        this.$content.html(msg)
    };
    MiniDialog.prototype.getWrapper = function(msg){
       return this.$content;
    };
	...
	out.prompt = function(msg,defaultPrompt,okFunction,cancelFunction,options,noRemoveWhenClose){
        ...
        var cbval = 'miniDialog_prompt_input';
		return new MiniDialog($.extend({
			content:['<div style="margin-bottom:5px">',msg,'</div><div><input class="',inputClass,'" cb-node="',cbval,'" style="width:175px;padding:6px 4px" value="',defaultPrompt,'"/></div>'].join(''),
			buttons:[
				{
					'value':'确定',
					'click':function(e,miniDialogObjectInstance){
